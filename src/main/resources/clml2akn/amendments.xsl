...
<xsl:template name="para-with-amendment">
	...
	<xsl:choose>
		...
		<xsl:when test="*[1][self::Text[child::InlineAmendment]] and *[2][self::BlockAmendment] and (empty(*[3]) or *[3][self::AppendText])">
			<p>
				<mod>
			...
			<xsl:apply-templates select="*[position() gt 3]" />
		</xsl:when>
		<xsl:when test="*[1][self::Text] and *[2][self::BlockAmendment] and (empty(*[3]) or *[3][self::AppendText])">
			<p>
				<xsl:apply-templates select="*[1]/node()" />
				<xsl:apply-templates select="*[2]">
